  ...
  const fetchData = async () => {
    ...
    try {
      if (currentView === 'aquarium') {
        ...
        // 프론트엔드에서 칸 3개 추가: 거북이 1개 (없는 경우만) + 빈칸 2개
        ...
        const emptySlots: UIFish[] = Array.from({ length: 2 }, (_, index) => ({
          fish_id: 1000 + index, // 고유 ID
          fish_name: '???',
          grade: 'LEGENDARY' as FishGrade,
          current_count: 0,
          is_owned: false,
          size: getFishSize('LEGENDARY' as FishGrade)
        }));

        setFishList([...converted, ...additionalFish, ...emptySlots]);
        setStats({ current: data.collected_count, total: data.total_fish + additionalFish.length + 2 });
      }
    } catch (err) {
