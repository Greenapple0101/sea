...
const DEFAULT_GACHA_COST = 10;

export function StudentGacha() {
  const { user, isAuthenticated, userType, access_token, updateUser } = useAuth();
  ...
  const [gachaCost, setGachaCost] = useState(DEFAULT_GACHA_COST);
  const [studentCoral, setStudentCoral] = useState(0);
  ...
  const currentStudent = (userType === 'student' ? (user as StudentUser | null) : null);
  ...
  useEffect(() => {
    ...
    const fetchGachaInfo = async () => {
      ...
      try {
        const [gachaRes, meRes] = await Promise.all([
          get('/api/v1/gacha/info').catch(() => null),
          get('/api/v1/auth/student/me').catch(() => null)
        ]);

        let coralLoaded = false;
        let costLoaded = false;

        // 학생 잔액 우선 user context → API → fallback 0
        let resolvedCoral = currentStudent?.coral ?? 0;
        if (meRes && meRes.ok) {
          const meJson = await meRes.json();
          if (meJson?.data) {
            resolvedCoral = meJson.data.coral ?? resolvedCoral;
            coralLoaded = true;
            if (updateUser) {
              updateUser({
                coral: meJson.data.coral ?? resolvedCoral,
                research_data: meJson.data.research_data ?? currentStudent?.research_data ?? 0
              });
            }
          }
        } else if (resolvedCoral > 0) {
          coralLoaded = true;
        }
        if (isMounted) {
          setStudentCoral(resolvedCoral);
        }

        // 가챠 비용: API → 기본값
        if (gachaRes && gachaRes.ok) {
          const gachaJson = await gachaRes.json();
          if (gachaJson?.success) {
            if (isMounted) {
              setGachaCost(gachaJson.data?.gacha_cost ?? DEFAULT_GACHA_COST);
            }
            costLoaded = true;
          }
        }
        if (!costLoaded) {
          if (isMounted) {
            setGachaCost(DEFAULT_GACHA_COST);
          }
          costLoaded = true;
        }

        if (!coralLoaded && !costLoaded) {
          throw new Error('가챠 정보를 불러올 수 없습니다.');
        }
      } catch (err) {
        if (isMounted) {
